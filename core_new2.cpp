...
// and says to replace them after a simulation pass. Ways evaluation uses the
// paytable and all-ways symbol-pay logic from core/core.cpp as requested.
...
const int paying_symbol_count = 8;
constexpr int max_match = 6;

const Symbol payingSymbols[paying_symbol_count] = {HV1, HV2, HV3, HV4, LV1, LV2, LV3, LV4};

// Source: core/core.cpp PayTable. Pay values are symbol-pay credits, multiplied
// by the number of left-to-right ways found in evaluateWaysWins().
const int PayTable[paying_symbol_count][max_match] = {
...
int weightedRestrictedOddReel() {
    // Source: Game_Rules/BaseGame_1.json and Free_Spins_3.json.
    // Reels 1, 3, and 5 in the rules are 0-indexed reels 0, 2, and 4 here.
    // Table G weights are preserved proportionally across the allowed reels.
    return pickWeightedInt({{0, 2200}, {2, 2600}, {4, 2000}});
}

bool requiresOddReelRestriction(GameState state, Symbol symbol) {
    if (state == BASE_GAME) {
        return symbol == HAZARD_HARD_HAT;
    }
    if (state == NORMAL_FREE_SPINS) {
        return symbol == HAZARD_HARD_HAT ||
               symbol == HORIZONTAL_SAW_HARD_HAT ||
               symbol == VERTICAL_SAW_HARD_HAT;
    }
    return false;
}
...
void placeHardHats(Window& window, GameState state, int active_rows) {
    ...
    for (int i = 0; i < count; i++) {
        Symbol placed_symbol = state == BASE_GAME ? baseHardHatType(active_rows)
                                                  : featureHardHatType(state);
        bool placed = false;
        for (int attempt = 0; attempt < 20 && !placed; attempt++) {
            int row = weightedRow(active_rows);
            int reel = requiresOddReelRestriction(state, placed_symbol)
                ? weightedRestrictedOddReel()
                : weightedReel();
            if (row < startRow(active_rows)) continue;
            if (isHardHat(window[row][reel])) continue;

            window[row][reel] = placed_symbol;
            placed = true;
        }
    }
}
...
long long evaluateWaysWins(const Window& window, int active_rows) {
    // Source: core/core.cpp waysWinCalculation().
    // Paying symbols are seeded from the first active reel only. WILD does not
    // seed its own win but substitutes for each seeded paying symbol.
    long long win = 0;
    vector<int> first_reel_counts(paying_symbol_count, 0);

    for (int row = startRow(active_rows); row < no_of_rows; row++) {
        Symbol s = window[row][0];
        if (isPayingSymbol(s)) {
            first_reel_counts[static_cast<int>(s)]++;
        }
    }

    vector<Symbol> unique_symbols;
    for (int i = 0; i < paying_symbol_count; i++) {
        if (first_reel_counts[i] > 0) unique_symbols.push_back(payingSymbols[i]);
    }

    for (Symbol symbol : unique_symbols) {
        int left2right = 0;
        long long ways = 1;
        ...
        for (int reel = 0; reel < no_of_reels; reel++) {
            int occ_in_reel = 0;
            for (int row = startRow(active_rows); row < no_of_rows; row++) {
                if (window[row][reel] == symbol || window[row][reel] == WILD) {
                    occ_in_reel++;
                }
            }
            if (occ_in_reel <= 0) break;

            ways *= occ_in_reel;
            left2right++;
        }

        if (left2right < max_match) {
            win += static_cast<long long>(PayTable[static_cast<int>(symbol)][left2right]) * ways;
        }
    }
...
FeatureResult playFreeGames(GameState feature_state,
                            ...
                            const vector<TriggerSymbol>& trigger_positions,
    ...
    for (const auto& pos : trigger_positions) {
        frames[pos.row][pos.reel] = STRAW_FRAME;
    }

    // Source: Game_Rules/Free_Spins_1.json and SuperSaw_FS_Tables setup note.
    // Trigger-window saw symbols execute once before the first free spin.
    for (const auto& pos : trigger_positions) {
        if (isSawHat(pos.symbol)) {
            result.overflow_win += applySawPath(frames, pos.row, pos.reel, active_rows, pos.symbol);
        }
    }
...
SpinResult spinBaseGame(const vector<ReelSet>& reelsets, Statistics& stats) {
    ...
    if (trigger_count >= 6) {
        vector<pair<int, int>> trigger_cells = hardHatPositions(window, active_rows);
        vector<TriggerSymbol> trigger_positions;
        for (const auto& pos : trigger_cells) {
            trigger_positions.push_back({pos.first, pos.second, window[pos.first][pos.second]});
        }
...
void printResults(const Statistics& stats, const GameConfig& config, ostream& out) {
    ...
    out << "| Overflow prizes (included in feature totals) | " << stats.overflow_win << " | " << formatPercent(rtp(stats.overflow_win, stats)) << " |\n";
    out << "| Jackpot prizes (included in feature totals) | " << stats.jackpot_win << " | " << formatPercent(rtp(stats.jackpot_win, stats)) << " |\n\n";
    out << "Overflow and jackpot rows are subcomponent counters already included in the Normal/Super Saw feature totals.\n\n";
    ...
    out << "- Base reelset usage R1-R4 (R1 is the 3-row base spin; R2-R4 are not re-spun after Hazard expansion): "
    ...
    out << "- Ways wins use the paytable and all-ways symbol-pay logic from `core/core.cpp`.\n";
